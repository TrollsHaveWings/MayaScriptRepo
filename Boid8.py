...
class UI:
    ...
    def create_flock_btn_active(self, *args):
        ...
        Flock(self.flock_params).create_flock()
    ...
    def bake_simulation_btn_active(self, *args):
        ...
        Flock(self.flock_params).animate_flock()
    ...
    def update_flock_params(self):
        ...
        self.flock_params['collision_objects'] = cmds.optionMenu(self.collision_objects_menu, query=True, value=True)
...
class Flock:
    ...
    def update_boid(self, boid, frame):
        print(f"Boid: {boid.boid_name}, Frame: {frame}")
        print(f"Position: {boid.position}")
        print(f"Velocity: {boid.velocity}")
        ...
        print(f"New Position: {cmds.getAttr(boid.boid_name + '.translate')[0]}")
        cmds.setKeyframe(boid.boid_name, t=frame)
        print(f"Keyframe set for {boid.boid_name} at frame {frame}")
